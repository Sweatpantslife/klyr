...
6. **Initiation and paralysis.** Starting is the single point where executive, emotional, and reward systems fail *simultaneously* — once a task is underway it supplies its own stimulation, but the start is the expensive step, and roughly 75% of adults with ADHD qualify as chronic procrastinators versus ~35% of controls. Vague tasks are silent blockers, yet decomposition is itself an impaired executive function — "just break it down" outsources the hardest step to the person least equipped for it in that moment. Deep dive: [task-initiation-and-paralysis](daily-life/task-initiation-and-paralysis.md).
